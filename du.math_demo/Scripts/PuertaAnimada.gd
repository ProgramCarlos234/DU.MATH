...
func _process(delta: float) -> void:
	if movimiento_abrir:
		...
		# Movimiento hacia abajo (cerrar)
		mover_muro(muro_entrada, Limite_movimiento_muro_abajo.position, delta)
		mover_muro(muro_entrada_2, Limite_movimiento_muro_arriba.position, delta)
		
		if muro_entrada.position.distance_to(Limite_movimiento_muro_abajo.position) < 1.0 \
		and muro_entrada_2.position.distance_to(Limite_movimiento_muro_arriba.position) < 1.0:
